...
class MY_Model extends CI_Model {
	...
	protected function get_list_set_input($input = array())
	{
		...
		//(vi du $input['order'] = array('id','DESC'))
		...
		{
			//danh sach sap xep theo id giam dan 
			$order = ($this->order == '') ? array($this->table.'.'.$this->key, 'desc') : $this->order;
			$this->db->order_by($order[0], $order[1]);
		}
		
		// them dieu kien limit cho cau truy van qua $input['limit'] 
		//(vi du $input['limit'] = array('10' ,'0')) 
	...
	/**
	 * kiem tra ton tai dư lieu theo 1 dieu kien nao do
